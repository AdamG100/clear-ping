...
- **Package Management:** This project uses **npm** for managing dependencies. All package installations and scripts should use `npm`
...
- **do not prompt to run `npm run dev` each time a server file is changed. This is assumed knowledge.**
- **do not prompt to run `npm run build` each time a production build is needed. This is assumed knowledge.**
